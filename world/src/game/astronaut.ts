...
export type MoveMode = 'idle' | 'run' | 'jump' | 'swim';
...
interface JointCur {
  shL: { x: number; z: number }; shR: { x: number; z: number };
  elL: number; elR: number;
  hipL: number; hipR: number;
  kneeL: number; kneeR: number;
  bodyPitch: number; bodyY: number; bodyRoll: number;
  headPitch: number;
}

export class Astronaut {
  readonly group = new THREE.Group();   // world position + heading
  private body = new THREE.Group();      // procedural bob / pitch
  ...
  // ---------------------------------------------------------------------------
  update(dt: number, mode: MoveMode, speed01: number): void {
    this.time += dt;
    const t = this.time;
