...
// A nebula reads nothing from its painting but a 64x64 grid of colour
// (extractPalette below), so it loads a 128 px thumbnail instead of the
// painting itself — a couple of kB against a couple of hundred, across two
// dozen images. The thumbnails are generated by scripts/optimize-images.mjs
// and keyed by the same filename; a painting added without running that
// script simply falls back to the full-size file on the CDN and still works.
//
// These stay bundled where the paintings themselves do not: the whole set is
// 152 kB, and unlike a painting every one of them is read on a first visit.
const THUMBS = Object.fromEntries(
  Object.entries(
    import.meta.glob('/artgallery/thumbs/*.{png,jpg,jpeg,webp}', {
      eager: true,
      query: '?url',
      import: 'default',
    })
...
//
// Deliberately NOT started from initPaintingNebulae: every one of these images
// is a full painting, downloaded and decoded to sample a 64x64 palette, and
// running that during module eval put two dozen of them between the player and
// the LAUNCH button. game.js starts the queue once the game is actually
// running (idle callback after launch). Until a painting's palette arrives its
// nebula keeps the placeholder colour it already had, which is the same thing
// that happens today on a slow connection.
let queueStarted = false;

export function startPaintingNebulaBuild() {
  ...
  const step = () => {
    ...
    const img = new Image();
    // extractPalette reads the pixels back out of a canvas, which a
    // cross-origin draw would forbid. The thumbnails are same-origin, but the
    // fallback path (a painting added without running the optimize script)
    // reaches the CDN, and jsDelivr answers with Access-Control-Allow-Origin.
    img.crossOrigin = 'anonymous';
    img.onload = () => {
      // decode() first: drawImage on an undecoded image decodes it inline on
      // the main thread, which for a multi-megapixel painting is a visible
      // stutter mid-flight. Awaiting the decode moves that off-thread and the
      // drawImage below becomes a copy.
      img
        .decode()
        .catch(() => {}) // not supported / already failed — draw anyway
        .then(() => {
          try {
...
export function initPaintingNebulae(scene) {
  const entries = GALLERY_FILES.filter(
    (file) => !UNTITLED_RE.test(file) && !SKIP_FILES.has(file)
  ).map((file) => {
    const url = galleryUrl(file);
    return { file, url, paletteUrl: THUMBS[file] ?? url };
  });

  const takenDirs = [];
  for (const { file, url, paletteUrl } of entries) {
    ...
    paintingNebulae.push({ record, file, url, paletteUrl, seed, dir, distance, radius });
  }
}
